...
function computePoidsPoints(poidsReel: number, poidsVol: number) {
  ...
  // ── Échelle dynamique ─────────────────────────────────────────────────────
  // Circumradius du triangle équilatéral = R * sqrt(ratio)
  // → quand ratio > 1 le triangle dépasse du cercle, quand ratio = 1 il y est inscrit
  const half  = Math.min(PA_SIZE / 2, PA_VIZ_H / 2);              // 97.5 px
...
function drawPoidsFrame(
  ...
  const cx = PA_SIZE / 2;
  const cy = PA_VIZ_H  / 2;   // centre de la zone formes (y=0..PA_VIZ_H)
  ...
  // Le canvas a l'axe Y inversé par rapport à matplotlib → on applique -Y
  ctx.beginPath();
  ...
  // ── Labels (même information que les annotations Python) ─────────────────
  ctx.font = `600 14px 'Outfit','Segoe UI',sans-serif`;
  ctx.textAlign    = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillStyle = col.lC;
  ctx.fillText(`● Poids réel : ${pr.toFixed(2)} kg`,        cx, PA_VIZ_H + 26);
  ctx.fillStyle = col.lT;
  ctx.fillText(`▲ P. volumétrique : ${pv.toFixed(2)} kg`,   cx, PA_VIZ_H + 54);
}
...
function createParcelCard(parcel: GuestParcel, index: number): string {
  ...
  if (hasResult) {
    ...
    <div class="anim-poids-wrapper"
      ...
      <canvas class="anim-poids-canvas" width="400" height="400"></canvas>
    </div>`;
...
function genererSvgColisDynamique(longueur: number, largeur: number, hauteur: number): string {
    ...
    <div class="icone-colis-wrapper">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="${minX} ${minY} ${targetW} ${targetH}" class="rotate-3d" fill="none" stroke="var(--primary)" stroke-width="${strokeW}" stroke-linecap="round" stroke-linejoin="round" style="width: 100%; height: 100%; overflow: visible;">
            <polygon points="${p0x},${p0y} ${p1x},${p1y} ${p6x},${p6y} ${p4x},${p4y} ${p5x},${p5y} ${p3x},${p3y}" />
